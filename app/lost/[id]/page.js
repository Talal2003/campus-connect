...
export default function ItemDetail({ params }) {
  ...
  // Format location to show room number if available
  const locationDisplay = item.room_number 
    ? `${item.location} (Room ${item.room_number})` 
    : item.location;
      ...
      <div className="card" style={{ 
        ...
        <div style={{ 
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
            {/* Report Found Button - Only show if not the owner and status is still pending */}
            {!isOwner && item.status === 'pending' && (
              <Link 
                href={`/report/found?title=${encodeURIComponent(item.title || '')}&category=${encodeURIComponent(item.category || '')}&description=${encodeURIComponent(item.description || '')}&location=${encodeURIComponent(item.location || '')}&room_number=${encodeURIComponent(item.room_number || '')}&date=${encodeURIComponent(item.date || '')}&image_url=${encodeURIComponent(item.image_url || '')}&reference_id=${encodeURIComponent(item.id || '')}`}
                style={{
            ...
            <div style={{ 
              ...
              <div style={{ lineHeight: '1.8' }}>
                ...
                <p><strong>Date Lost:</strong> {new Date(item.date).toLocaleDateString()}</p>
                ...
                <p>
                  ...
                  {isOwner ? (
                    ...
                    >
                      <option value="pending">Still Lost</option>
                      <option value="found">Found</option>
                      <option value="claimed">Claimed by Owner</option>
                    </select>
                  ) : (
                    <span style={{
                      backgroundColor: item.status === 'found' ? '#fff3cd' : 
                                       item.status === 'claimed' ? '#d4edda' : '#cce5ff',
                      color: item.status === 'found' ? '#856404' : 
                             item.status === 'claimed' ? '#155724' : '#004085',
                      padding: '0.25rem 0.5rem',
                      borderRadius: '4px',
                      fontSize: '0.9rem',
                      fontWeight: '500'
                      ...
                      {item.status === 'found' ? 'Found' : 
                       item.status === 'pending' ? 'Still Lost' : 
                       item.status === 'claimed' ? 'Claimed by Owner' : 
                       item.status === 'delivered' ? 'Returned to Owner' : 
                       item.status}
                    </span>
                  )}
